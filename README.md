...
```
Install ```pytorch=2.2.0, cuda 12.1``` version from pytorch org first, and then install other essential resources
```
...
After downloading, place the data in ```data/Matzoo```

## Training 

### Train Super-Resolution
Train Share with super-resolution on the Cave dataset under a $\times 2$ downsampling ratio; run
```
bash scripts/run.sh sr train
```
Train Share with super-resolution on the PaviaUni dataset under a $\times 2$ downsampling ratio; modify the dataset name in run.sh


### Train Inpainting
Train Share with inpainting on the Chikusei dataset; run
```
bash scripts/run.sh inp train
```
**index** means which tile from full Chikusei, **mat_index** means which mask shape; modify them in run.sh
